...
function checkPassword(password, stored) {
  ...
  return (
    candidate.length === Buffer.from(hash, "hex").length &&
...
export function demoData(today = new Date(), password) {
  ...
  const projects = [
    {
      id: "p1",
      key: "NDS",
      name: "Сайт для клиента «GreenStone»",
      description: "Корпоративный сайт: дизайн, вёрстка, интеграция с CRM.",
      status: "active",
      leadId: "u2",
      memberIds: ["u2", "u3", "u4", "u7"],
      startDate: day(-30),
      dueDate: day(25),
      allowEmployeeTasks: true,
      createdAt: day(-30),
    },
    {
      id: "p2",
      ...
      description: "Кампания на осень: стратегия, креативы, аналитика.",
      status: "active",
      leadId: "u2",
      memberIds: ["u2", "u6", "u8"],
      startDate: day(-14),
    ...
    },
    {
      ...
      description: "CRM-модуль, интеграции, отчёты по воронке.",
      status: "planning",
      leadId: "u1",
      memberIds: ["u1", "u5", "u8", "u7"],
      startDate: day(-7),
      dueDate: day(45),
      allowEmployeeTasks: true,
      createdAt: day(-7),
  ...
  const spec = [
    [
      "Собрать структуру страниц и прототип",
      "p1",
      "u3",
      "in_progress",
      "high",
    ...
    [
      "Тестирование вёрстки на мобильных",
      "p1",
      "u7",
      "backlog",
      "medium",
    ...
    [
      "Аудит конкурентов и позиционирование",
      "p2",
      "u6",
      "done",
      "high",
    ...
    [
      "Собрать требования отдела продаж",
      "p3",
      "u8",
      "in_progress",
      "high",
    ...
    [
      "Обновить регламент работы с лидами",
      "p3",
      "u1",
      "backlog",
      "low",
    ...
    [
      "Проверить доступность и скорость",
      "p1",
      "u7",
      "backlog",
      "medium",
    ...
    [
      "Подготовить инструкцию для менеджеров",
      "p3",
      "u2",
      "backlog",
      "low",
    ...
    [
      "Зафиксировать метрики кампании",
      "p2",
      "u6",
      "in_progress",
      "high",
  ...
  for (const [
    ...
    tasks.push({
      ...
      attachments:
        n % 4 === 0
          ? [{ title: "Макет в Figma", url: "https://figma.com/file/demo" }]
          : [],
    ...
    notifications: [
      {
        ...
        taskId: "t3",
        type: "assigned",
        text: "Вам назначена задача NDS-103",
        at: day(-1),
        read: false,
      },
      {
        ...
        taskId: "t6",
        type: "overdue",
        text: "Задача NDS-106 просрочена",
        at: day(-1),
        read: false,
      },
      {
        ...
        taskId: "t10",
        type: "status",
        text: "Задача NDA-110 переведена в работу",
        at: day(-2),
        read: true,
      },
    ],
...
const isMember = (user, project) =>
  Boolean(project) &&
  (project.memberIds.includes(user.id) || project.leadId === user.id);
const leadsProject = (user, project) =>
  Boolean(project) && project.leadId === user.id;
const canSeeProject = (user, project) =>
  isAdmin(user) || isMember(user, project);
const canManageProject = (user, project) =>
  isAdmin(user) || leadsProject(user, project);
const canEditTask = (user, task, project) =>
  isAdmin(user) || canManageProject(user, project) || task.authorId === user.id;
const canMoveTask = (user, task, project) =>
  canEditTask(user, task, project) || task.assigneeId === user.id;

/** Уведомление в приложении (ТЗ, п. 17). */
function notify(state, userId, taskId, type, text) {
  state.notifications.push({
    id: `n${state.notifications.length + 1}-${Date.now()}`,
    userId,
    taskId,
    type,
    text,
...
function writeHistory(state, taskId, userId, field, from, to) {
  state.history.push({
    id: `h${state.history.length + 1}-${Date.now()}`,
    taskId,
    userId,
    at: now(),
    field,
...
function userLoad(state, userId, today) {
  const own = state.tasks.filter(
    (t) => t.assigneeId === userId && t.status !== "done",
  );
  const overdue = own.filter((t) => daysLeft(t.dueDate, today) < 0);
  return {
    activeTasks: own.length,
    overdueTasks: overdue.length,
    load: Math.min(100, Math.round((own.length / 10) * 100)),
  };
}
...
export function createApp({ store, today = () => new Date() }) {
  ...
  function visibleProjects(user) {
    ...
    return isAdmin(user)
      ? s.projects
      : s.projects.filter((p) => canSeeProject(user, p));
  }
  ...
  const projectStats = (project, day) => {
    ...
    return {
      ...
      overdueTasks: tasks.filter(
        (t) => t.status !== "done" && daysLeft(t.dueDate, day) < 0,
      ).length,
  ...
  route("POST", "/api/users", ({ body, user }) => {
    if (!isAdmin(user))
      return error(403, "Пользователей создаёт администратор");
    const s = state();
  ...
  route("DELETE", "/api/users/:id", ({ params, user }) => {
    if (!isAdmin(user))
      return error(403, "Удалять пользователей может администратор");
    const s = state();
    ...
    return {
      ...
      body: {
        projects: list.map((p) => ({
          ...p,
          lead: publicUser(
            state().users.find((u) => u.id === p.leadId) ?? {
              id: null,
              name: "—",
              position: "",
              email: "",
  ...
  route("POST", "/api/projects", ({ body, user }) => {
    if (!isAdmin(user))
      return error(403, "Создавать проекты может администратор");
    const s = state();
    const name = String(body.name || "").trim();
    if (!name) return error(400, "Нужно название проекта");
    const project = {
      id: `p${s.projects.length + 1}`,
      key: String(body.key || name.slice(0, 3))
        .toUpperCase()
        .slice(0, 4),
      name,
      ...
      memberIds: Array.from(
        new Set(
          [body.leadId || user.id, ...(body.memberIds || [])].filter(Boolean),
        ),
      ),
  ...
  route("GET", "/api/projects/:id", ({ params, user }) => {
    ...
    if (!canSeeProject(user, project))
      return error(403, "Нет доступа к проекту");
    const day = today();
    return {
      status: 200,
      body: {
        project: {
          ...project,
          lead: publicUser(
            s.users.find((u) => u.id === project.leadId) ?? {
              id: null,
              name: "—",
            },
          ),
          members: project.memberIds.map((id) =>
            publicUser(s.users.find((u) => u.id === id) ?? { id, name: "—" }),
          ),
  ...
  route("PATCH", "/api/projects/:id", ({ params, body, user }) => {
    ...
    if (!canManageProject(user, project))
      return error(403, "Нет прав на изменение проекта");
    ...
    if (body.allowEmployeeTasks !== undefined)
      project.allowEmployeeTasks = Boolean(body.allowEmployeeTasks);
    if (body.leadId && isAdmin(user)) project.leadId = body.leadId;
    if (body.memberIds && Array.isArray(body.memberIds))
      project.memberIds = Array.from(new Set(body.memberIds));
    store.save();
  ...
  route("POST", "/api/projects/:id/members", ({ params, body, user }) => {
    ...
    if (!project) return error(404, "Проект не найден");
    if (!canManageProject(user, project))
      return error(403, "Добавлять участников может руководитель проекта");
  ...
  route("DELETE", "/api/projects/:id/members/:userId", ({ params, user }) => {
    ...
    if (!project) return error(404, "Проект не найден");
    if (!canManageProject(user, project))
      return error(403, "Убирать участников может руководитель проекта");
  ...
  route("GET", "/api/tasks", ({ user, query }) => {
    ...
    if (query.assigneeId)
      list = list.filter((t) => t.assigneeId === query.assigneeId);
    if (query.overdue === "1")
      list = list.filter(
        (t) => t.status !== "done" && daysLeft(t.dueDate, day) < 0,
      );
    if (query.due === "week")
      list = list.filter(
        (t) =>
          t.status !== "done" &&
          daysLeft(t.dueDate, day) >= 0 &&
          daysLeft(t.dueDate, day) <= 7,
      );
    if (query.q) {
      const needle = norm(query.q);
      list = list.filter(
        (t) => norm(t.title).includes(needle) || norm(t.key).includes(needle),
      );
    }
    return {
      status: 200,
  ...
  route("POST", "/api/tasks", ({ body, user }) => {
    ...
    const mayCreate =
      isAdmin(user) ||
      canManageProject(user, project) ||
      (project.allowEmployeeTasks && project.memberIds.includes(user.id));
    if (!mayCreate)
      return error(403, "В этом проекте сотрудники не создают задачи");
  ...
  route("GET", "/api/tasks/:id", ({ params, user }) => {
    ...
    if (!canSeeProject(user, project))
      return error(403, "Нет доступа к задаче");
    return {
      status: 200,
      body: {
        task: {
          ...task,
          overdue:
            task.status !== "done" && daysLeft(task.dueDate, today()) < 0,
          project: { id: project.id, name: project.name },
          assignee: task.assigneeId
            ? publicUser(s.users.find((u) => u.id === task.assigneeId))
            : null,
  ...
  route("PATCH", "/api/tasks/:id", ({ params, body, user }) => {
    ...
    if (!mayEditFields && !mayMove)
      return error(403, "Нет прав на изменение задачи");
    const touchesFields = [
      "title",
      "description",
      "priority",
    ...
    if (touchesFields && !mayEditFields)
      return error(403, "Поля задачи правит автор или руководитель проекта");
    ...
    if (body.projectId !== undefined && body.projectId !== task.projectId) {
      ...
      if (!canSeeProject(user, next))
        return error(403, "Нет доступа к новому проекту");
    ...
    if (
      ...
      if (!mayMove)
        return error(403, "Менять статус может исполнитель или руководитель");
      ...
      if (task.assigneeId && task.assigneeId !== user.id)
        notify(
          ...
          task.id,
          "status",
          `Задача ${task.key}: ${STATUS_TITLES[body.status]}`,
        );
    }
    task.updatedAt = now();
    store.save();
  ...
  route("POST", "/api/tasks/:id/status", ({ params, body, user }) => {
    ...
    if (!STATUSES.includes(body.status))
      return error(400, "Неизвестный статус");
        ...
        columns: STATUSES.map((status) => ({
          ...
          tasks: s.tasks
            ...
            .map((t) => ({
              ...
              overdue: t.status !== "done" && daysLeft(t.dueDate, day) < 0,
              assignee:
                s.users.find((u) => u.id === t.assigneeId)?.name ?? null,
              comments: s.comments.filter((c) => c.taskId === t.id).length,
  ...
  route("POST", "/api/tasks/:id/comments", ({ params, body, user }) => {
    ...
    const watchers = [task.assigneeId, task.authorId, project.leadId].filter(
      (id) => id && id !== user.id,
    );
    for (const id of watchers)
      notify(
        ...
        task.id,
        "comment",
        `Новый комментарий в задаче ${task.key}`,
      );
    store.save();
  ...
  route("GET", "/api/notifications", ({ user }) => {
          ...
          return [
            {
              ...
              taskId: t.id,
              type: "due_soon",
              text: `Срок задачи ${t.key} — через ${left} дн.`,
              at: t.dueDate,
              read: false,
              derived: true,
            },
    ...
    return {
      status: 200,
      body: { notifications: all, unread: all.filter((n) => !n.read).length },
    };
  });
  ...
  route("GET", "/api/dashboard", ({ user }) => {
    ...
    const thisWeek = active.filter(
      (t) => daysLeft(t.dueDate, day) >= 0 && daysLeft(t.dueDate, day) <= 7,
    );
          ...
          isAdmin(user) || isPm(user)
            ? {
                ...
                byStatus: STATUSES.map((status) => ({
                  status,
                  title: STATUS_TITLES[status],
                  count: teamTasks.filter((t) => t.status === status).length,
                })),
              }
            : null,
      },
  ...
  route("GET", "/api/search", ({ user, query }) => {
    ...
    if (!q)
      return { status: 200, body: { projects: [], tasks: [], users: [] } };
    const s = state();
    const projects = visibleProjects(user).filter(
      ...
      .filter(
        (t) =>
          ...
          (isAdmin(user) ||
            canManageProject(
              user,
              s.projects.find((p) => p.id === t.projectId),
            ) ||
            t.assigneeId === user.id ||
            t.authorId === user.id),
      )

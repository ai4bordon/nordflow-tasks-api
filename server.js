...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/') {
    return send(200, {
      ...
      endpoints: [
        ...
        'GET /api/dashboard', 'GET /api/users', 'POST /api/users',
        'PATCH /api/users/:id', 'DELETE /api/users/:id',
        'GET /api/projects', 'POST /api/projects', 'GET /api/projects/:id',
        'PATCH /api/projects/:id', 'POST /api/projects/:id/archive',
        ...
        'POST /api/tasks/:id/comments', 'PATCH /api/comments/:id', 'DELETE /api/comments/:id',
  ...
  if (!pathname.startsWith('/api/')) return send(404, { error: 'Неизвестный адрес' });

  const body = req.method === 'GET' || req.method === 'DELETE' ? {} : await readBody(req);
  if (body === null) return send(400, { error: 'Некорректный JSON в теле запроса' });
